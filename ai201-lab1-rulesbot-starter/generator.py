...
def generate_response(query, retrieved_chunks):
    ...
    context = "\n".join(
        f"Game: {chunk['game']}\nRule: {chunk['text']}" for chunk in retrieved_chunks
    )

    # Generate the response
    response = (
        f"Based on the rules I found:\n\n{context}\n\n"
        "If this doesn't answer your question, try rephrasing it or checking the rules directly."
    )
    return response